...
import type { Prisma, WalletHold } from '@prisma/client';
...
export class WalletService {
   ...
   *
   * ۴۰۲ می‌دهد اگر موجودی نرسد یا پاکتِ گروه جا نداشته باشد — هر دو **پیش
   * از** هر فراخوانِ پولی، که تمامِ نکتهٔ وارونگیِ بودجه است.
   */
  async hold(
    userId: string,
    amount: number,
    ...
    if (!opts.purpose?.trim()) {
      throw new BadRequestException('هر رزرو باید دلیلِ خوانا داشته باشد.');
    }
   ...
   */
  async settle(holdId: string, actualAmount: number, opts: { generationId?: string; note?: string } = {}) {
    if (!Number.isInteger(actualAmount) || actualAmount < 0) {
      throw new BadRequestException('خرجِ واقعی باید عددِ صحیحِ نامنفی باشد.');
    }

    return this.tx(async (tx) => {
      const hold = await tx.walletHold.findUnique({ where: { id: holdId } });
      if (!hold) throw new NotFoundException('رزرو پیدا نشد.');
      // بی‌اثر در تکرار: تسویهٔ دوباره همان نتیجهٔ قبلی را برمی‌گرداند.
      if (hold.status === 'SETTLED') return { hold, noop: true };
      if (hold.status === 'RELEASED') {
        throw new BadRequestException('این رزرو آزاد شده — دیگر تسویه‌پذیر نیست.');
      }
   ...
   *
   * `estimatedCost = 0` (مثلِ کلِ مسیرِ TEXT) هیچ رزروی نمی‌سازد ولی از همین
   * مسیر رد می‌شود — یعنی روزی که همان گام پولی شد، فقط عددش عوض می‌شود.
   */
  async runWithHold<T>(
  ...
  private async reserveInBudget(tx: Prisma.TransactionClient, jobGroupId: string, amount: number) {
  ...
  private assertPositive(amount: number) {
    if (!Number.isInteger(amount) || amount <= 0) {
      throw new BadRequestException('مبلغ باید عددِ صحیحِ مثبت باشد (واحد: سکه).');
    }
  }
